...
import { TypeBody, StyledInternalLink } from '../../theme'
import DoubleCurrencyLogo from '../DoubleLogo'
import { JSBI , TokenAmount } from '@pancakeswap-libs/sdk' 
  ...
  return (
    ...
    <Wrapper showBackground={isStaking} bgColor={backgroundColor}>
      ...
      <TopSection>
        ...
        <TypeBody.white fontWeight={600} fontSize={24} style={{ marginLeft: '8px' }}>
      ...
      <StatContainer>
        <RowBetween>
          <TypeBody.white> Total deposits</TypeBody.white>
          <TypeBody.white>
        ...
        <RowBetween>
          <TypeBody.white> Total Rewards </TypeBody.white>
          <TypeBody.white>{`$${ parseInt(rewards.
            toFixed(0)).toLocaleString()} / day`}</TypeBody.white>
        </RowBetween>
        <RowBetween>
          <TypeBody.white> Pool rate </TypeBody.white>
          <TypeBody.white>{`${stakingInfo.totalRewardRate
            ?.toFixed(2, { groupSeparator: ',' }).replace(/[.,]00$/, "")} QUICK / day`}</TypeBody.white>
        </RowBetween>

        { 
          stakingInfo?.oneYearFeeAPY && stakingInfo?.oneYearFeeAPY > 0 && ( 
          <RowBetween>
          <TypeBody.white> Fees (24hr) </TypeBody.white>
          <TypeBody.white>{`$${parseInt(stakingInfo?.oneDayFee.toFixed(0)).toLocaleString()}`}</TypeBody.white>
        </RowBetween>)
        }
        ...
        { 
          stakingInfo?.oneYearFeeAPY && stakingInfo?.oneYearFeeAPY > 0 && ( 
          <RowBetween>
          <TypeBody.white> Rewards + Fee APY </TypeBody.white>
          <TypeBody.white>{`${apyWithFee}%`}</TypeBody.white>
        </RowBetween>)
        }
      </StatContainer>
      ...
      {isStaking && !stakingInfo.ended && (
        <>
          ...
          <BottomSection showBackground={true}>
            <TypeBody.black color={'white'} fontWeight={500}>
              <span>Your rate</span>
            </TypeBody.black>

            <TypeBody.black style={{ textAlign: 'right' }} color={'white'} fontWeight={500}>
